...
void drawOBJ(GLuint VAO, int nVertices, Shader shader, glm::vec3 position, int texID, glm::vec3 scale, float angle);
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		drawOBJ(VAO,  nVerts,  shader, positionD, texDest , glm::vec3(0.5, 0.5, 0.5), 40.0);
...
void drawOBJ(GLuint VAO, int nVertices,Shader shader, glm::vec3 position, int texID, glm::vec3 scale, float angle) {
	...
	Mesh obj;
	obj.initialize(VAO, nVertices, &shader, position, scale, angle);
